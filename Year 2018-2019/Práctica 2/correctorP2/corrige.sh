...
if test $numero -eq 10; then
  for fichero in $fuentes; do
   if test $fichero == Valores.java || test $fichero == Fruto.java || \
      ...
      test $fichero == Trifido.java || test $fichero == Inmune.java || \
      test $fichero == Zombie.java || test $fichero == Vehiculo.java || \
...
if test $numfuentes -eq 10; then
  if test $numero -gt 10; then
  ...
  continuar=false
  echo "Error, los ficheros fuente necesarios son Coordenada.java, Fruto.java, Persona.java, Planta.java, Huerta.java, Valores.java, Trifido.java, Inmune.java, Zombie.java y Vehiculo.java; 0"
fi
