...
if test $numero -eq 9; then
  for fichero in $fuentes; do
   if test $fichero == Rasgo.java || test $fichero == Personaje.java || \
    test $fichero == Jugador.java || test $fichero == PersonajeIncompletoException.java ||test $fichero == Juego.java || \
    test $fichero == RasgoNoValidoException.java || test $fichero == RasgoPreexistenteException.java || \
    test $fichero == TableroIncompletoException.java || test $fichero == PartidaGanadaException.java ; then 
...
if test $numfuentes -eq 9; then
  ...
  continuar=false
  echo "Error, los ficheros fuentes necesarios son Rasgo.java, Personaje.java, Jugador.java, Juego.java, PersonajeIncompletoException.java, RasgoNoValidoException.java, RasgoPreexistenteException.java, TableroIncompletoException.java y PartidaGanadaException.java; 0"
fi
