...
if test $numero -eq 4; then
  for fichero in $fuentes; do
   if test $fichero == Producto.java || test $fichero == Terreno.java || \
    test $fichero == Habitante.java || test $fichero == Mistico.java ; then 
...
if test $numfuentes -eq 4; then
  if test $numero -gt 4; then
  ...
  continuar=false
  echo "Error, los ficheros fuentes necesarios son Producto.java, Terreno.java, Habitante.java y Mistico.java; 0"
fi
