...
public class Romskip {
	...
	public String embarque(String s,Coordenada c) {
		...
		if(bodega!=null && mapa!=null && s!=null && c!= null && mapa.getFil()>=c.fil() && mapa.getCol()>=c.col() && mapa.getDim()>=c.dim()) {
			...
			aux=new Sector[mapa.getFil()][mapa.getCol()][mapa.getDim()];
			aux=mapa.getSector();
			primera=new Mercancia[aux[x][y][z].devuNumMercancias()];
			...
			for(int i=0; i<primera.length; i++) {
				if(s.equalsIgnoreCase(primera[i].getEtiqueta())) {
					...
					if(espnulos>0 && pesocupado<=carga) {bodega[posicion]=primera[i];}
					i=primera.length;
				}
			}
			posicion=0;
			...
			for(int p=1; p<ordena.length; p++)
				for(int j=ordena.length-1; j>=p; j--)
					if(ordena[j-1].getDimension()>ordena[j].getDimension()) {
						auxiliar=ordena[j-1];
						ordena[j-1]=ordena[j];
						ordena[j]=auxiliar;						
					}
	...
	public boolean verifica(Mercancia m) {
		...
		if(m!=null) {
			for(int i=0; i<bodega.length; i++) {pesototal=pesototal+bodega[i].getPeso();}
			if(pesototal<=carga) {
				for(int i=0; i<bodega.length; i++)
			...
			for(int i=1; i<bodega.length; i++)
				for(int j=bodega.length-1; j>=i; j--)
					if(bodega[j-1].getDimension()>bodega[j].getDimension()) {
						aux=bodega[j-1];
						bodega[j-1]=bodega[j];
						bodega[j]=null;
						bodega[j]=aux;
